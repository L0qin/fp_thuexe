...
class _PostTheCarState extends State<PostCar> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (vehicleId == null) {
        _showDialog(context, 'Lỗi', 'Không thể Chỉnh sửa xe.');
        return;
      }
      postImages(vehicleId.toString());
      // If everything was successful, show a success message
      _showDialog(context, 'Thành công', 'Xe đã được Chỉnh sửa thành công.');
    } catch (e) {
      // Log error and show an error message
      print('Error during vehicle registration: $e');
      _showDialog(context, 'Lỗi', 'Đã xảy ra lỗi trong quá trình Chỉnh sửa xe.');
    }
  }
